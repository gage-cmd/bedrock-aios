...
- New modules are scaffolded by the `scaffold-module` skill (`.claude/skills/scaffold-module/`), not by hand — it generates the backend module + config/schema/migration/RLS-test and the dashboard widget/tabs from convention templates, and auto-wires AppModule + the widget registry. Only manual steps left: apply the migration (no runner) and flesh out the stub logic. See [docs/module-scaffold.md](docs/module-scaffold.md).
